...
import 'package:feel_better_fixed/auth/gender_screen.dart';
...
class _AgeScreenState extends State<AgeScreen> {
  ...
  final List<String> _fieldLabels = ["Your Age"];
  final List<String> _hintTexts = ["Type your Age here"];
  ...
  void saveAge() async {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        await users.doc(user?.uid).update({'age': int.parse(_age.text)});
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Error saving age: $e")));
    } finally {
      setState(() {
        _isloading = false;
      });
